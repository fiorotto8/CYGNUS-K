#!/usr/bin/env python3
"""
Summarize Garfield transport-point diffusion over a 50 cm drift length.

Input CSVs are expected in GarfieldDiffusion/plots or GarfieldSim/plots and
must end in *_transport_points.csv. The script writes:

  - DetectorNumbers/diffusion_2sigma_50cm_summary.csv
  - DetectorNumbers/diffusion_2sigma_vs_field_by_gas.png
"""
...
def find_input_dir(repo_root: Path) -> Path:
    ...
    raise FileNotFoundError(
        "Could not find input folder. Expected GarfieldDiffusion/plots or GarfieldSim/plots."
    )


def compute_two_sigma(diffusion_coeff_sqrt_cm: float, drift_length_cm: float) -> float:
    """Return 2 sigma in cm for D in sqrt(cm) and drift length L in cm."""
...
def make_comparison_plots(df: pd.DataFrame, output_png: Path) -> None:
    ...
    ax_dl.set_title("Longitudinal diffusion (2 sigma)")
    ax_dt.set_title("Transverse diffusion (2 sigma)")
    ax_dl.set_xlabel("Electric field [V/cm]")
    ax_dt.set_xlabel("Electric field [V/cm]")
    ax_dl.set_ylabel("2 sigma diffusion over 50 cm [mm]")
    ax_dt.set_ylabel("2 sigma diffusion over 50 cm [mm]")
...
def main() -> None:
    ...
    for csv_file in csv_files:
        ...
        for _, r in df.iterrows():
            ...
            dl_2sigma_cm = compute_two_sigma(dl, DRIFT_LENGTH_CM)
            dt_2sigma_cm = compute_two_sigma(dt, DRIFT_LENGTH_CM)
            rows.append(
                {
                    "gas": gas_description,
                    "point": r["point"],
                    ...
                    "DL(sqrt(cm))": dl,
                    "DT(sqrt(cm))": dt,
                    "DL_2sigma(cm)": dl_2sigma_cm,
                    "DT_2sigma(cm)": dt_2sigma_cm,
                    "DL_2sigma(mm)": 10.0 * dl_2sigma_cm,
                    "DT_2sigma(mm)": 10.0 * dt_2sigma_cm,
                }
            )
